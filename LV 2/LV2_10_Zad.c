...
int findRowWithMostZeros(int** matrix, int rows, int cols);
...
int main() {
	...
	int rowIndex = findRowWithMostZeros(matrix, rows, cols);
...
int* generateRow(int size) {
	const int LOWERLIMIT = 0, UPPERLIMIT = 1;
...
int findRowWithMostZeros(int** matrix, int rows, int cols) {
	...
	for (i = 0; i < rows; i++) {
		...
		if (currentZerosCounter > maxZeros) {
			maxZeros = currentZerosCounter;
			maxIndex = i;
		}

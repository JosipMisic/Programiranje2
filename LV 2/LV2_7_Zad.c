...
int findMaxValueFrequency(int** matrix, int rows, int cols);
int findMaxValue(int** matrix, int rows, int cols);
...
int main() {
	...
	int maxValueFrequency = findMaxValueFrequency(matrix, rows, cols);
	printf("Max is present %d times.", maxValueFrequency);
...
int findMaxValueFrequency(int** matrix, int rows, int cols) {
	int maxValue = findMaxValue(matrix, rows, cols);
	int i, j, counter = 0;
	for (i = 0; i < rows; i++) {
		for (j = 0; j < cols; j++) {
